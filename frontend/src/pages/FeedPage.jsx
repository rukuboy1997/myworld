        ...
        {isLoading && posts.length === 0 ? (
          <div className="flex flex-col gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="glass-panel rounded-3xl p-6 h-64 animate-pulse bg-white/5"
              ></div>
            ))}
          </div>
        ) : error ? (
          <div className="glass-panel rounded-3xl p-10 text-center flex flex-col items-center gap-4 border-destructive/30">
            ...
            <button
              onClick={fetchFeed}
              className="mt-4 px-6 py-2 bg-secondary text-secondary-foreground rounded-full font-medium hover:bg-secondary/80 transition-colors"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="glass-panel rounded-3xl p-12 text-center flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground max-w-sm">
              There are no posts on the network yet. Be the first to share your
              world!
            </p>
          </div>
        ) : (
