...
export default function AuthModal() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "signup") {
        ...
        setInfo(
          `If an account exists for ${email.trim()}, a verification code has been sent. Check your inbox.`,
        );
        setMode("reset");
      } else if (mode === "reset") {
        await resetPassword({
  ...
  const inputCls =
    "w-full bg-secondary/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary";
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md glass-panel border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {error && (
            <div className="p-3 bg-destructive/20 border border-destructive/40 rounded-xl text-sm text-destructive-foreground">
              {error}
            </div>
          )}
          {info && (
            <div className="p-3 bg-primary/15 border border-primary/30 rounded-xl text-sm">
              {info}
            </div>
          )}
